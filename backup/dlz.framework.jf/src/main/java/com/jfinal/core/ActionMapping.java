...
final class ActionMapping {
	...
	Action getAction(String url, String[] urlPara) {
		...
		return getAction(url, url, urlPara);
	}
	
	private Action getAction(String allUrl,String dealUrl,String[] urlPara){
		int i = dealUrl.lastIndexOf(SLASH);
		if (i != -1) {
			String tempUrl=dealUrl.substring(0, i);
			Action action = mapping.get(tempUrl);
			if(action!=null){
				urlPara[0] = allUrl.substring(i + 1);
				return action;
			}else{
				return getAction(allUrl, tempUrl, urlPara);
			}
		}
		return null;
	}
