...
Boot 2 / Boot 3 应用推荐引入 `top.dlzio:dlz-caller-mybatis-spring-boot-starter:6.7.5`，无需手动 Bean，默认 INFO 输出。配置与自定义方式见 [Starter 指南](../4.框架与插件集成/4.2-MyBatis-SQL日志插件.md)。以下为基础插件的手动接入方式。
