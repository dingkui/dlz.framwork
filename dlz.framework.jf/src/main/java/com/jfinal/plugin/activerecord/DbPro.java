...
public class DbPro {
	...
	public void dealWithReult(String sql,DealRs r, Object... paras) throws SQLException {
		Connection conn = null;
		ResultSet rs=null;
		...
		try {
			...
			while (rs.next()) {
				...
				for (int i=1; i<=columnCount; i++) {
					...
					record.put(labelNames[i].substring(0, 1).toLowerCase()+labelNames[i].substring(1), value);
				}
				r.deal(record);
			}
		} catch (Exception e) {
	...
	public int[] batch(String sql, String columns, List modelOrRecordList, int batchSize) {
		...
		try {
			...
			throw new ActiveRecordException(e.fillInStackTrace());
		} finally {
			if (autoCommit != null)
				try {conn.setAutoCommit(autoCommit);} catch (Exception e) {LogKit.error(e.getMessage(), e);}
			config.close(conn);
