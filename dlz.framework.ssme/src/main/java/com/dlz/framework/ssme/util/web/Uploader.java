...
//		return new File(realPath).getParent() +"/" +path;
		String uploadPath=ConfigUtil.Configs.Sys.getImgUploadPath().replace("/", File.separator);
	...
	public String getUrl() {
		//try {
		//return URLEncoder.encode(ConfigUtil.getConfig("nginx.img.server")+"/"+this.url,"utf-8");
		//} catch (UnsupportedEncodingException e) {
		//	logger.error(e.getMessage(),e);
		//}
	  return getHttpUrl(this.url);//this.url;
	}
	public static String getHttpUrl(String url) {
		return ConfigUtil.getConfig("nginxServer")+"/"+url;//this.url;
	}
