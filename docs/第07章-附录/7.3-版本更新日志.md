...
- Caller MDC key 固定为 `dlz-caller`（`DlzCallerContext.mdcKey` 常量）
...
- 新增负索引支持
- 新增 JSONList 类
- 新增工具类集合（ValUtil, JacksonUtil, DateUtil, StringUtils, BeanUtil）（注：BeanUtil 在 v6.7.3 被 ConvertUtil 替代）
