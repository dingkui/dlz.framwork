...
import com.dlz.framework.springframework.tag.MyToolDirectiveTag;
import com.dlz.framework.springframework.tag.TagRun;
...
public class FreeMarkerRender extends Render {
    ...
    static void init(ServletContext servletContext, Locale locale, int template_update_delay) {
    	...
    	//添加自定义标签 mytool by dk
        config.setSharedVariable("mytool", new MyToolDirectiveTag());
	...
	public void render() {
		...
		data.put("sta", request.getContextPath()+"/statics");
		data.put("userInfo", ThreadHolder.getAuthInfo());
		data.put("tagRun", new TagRun());
