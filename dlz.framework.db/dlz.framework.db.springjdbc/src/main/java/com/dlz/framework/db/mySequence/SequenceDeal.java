 ...
 */
@Slf4j
public class SequenceDeal implements ISequenceMaker{
	...
	private void init() {
		...
		try{
			...
			log.error("SequenceDeal init Error:"+e.getMessage());
		}
	...
	private class _SeqDao {
		...
		public int updSequence(String seqName, long newValue) {
			return commService.excuteSql("update t_sequence set val=? where name=?", newValue, seqName);
		}

		public SequenceBo getSequence(String seqName) {
			return commService.getBean("select * from t_sequence where name=?", SequenceBo.class, seqName);
		}

		public List<SequenceBo> getAll() {
			return commService.getBeanList("select * from t_sequence", SequenceBo.class);
		}

		public int createSequence(SequenceBo bo) {
			InsertParaMap in = new InsertParaMap("t_sequence");
			in.addValues(new JSONMap(bo));
			return commService.excuteSql(in);
		}
	}
}