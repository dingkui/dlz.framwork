...
import com.dlz.jf.holder.ControllerHolder;
...
public class ActionHandler extends Handler {
	...
	private static final MyLogger log = MyLogger.getLogger(ActionHandler.class);
	...
	public final void handle(String target, HttpServletRequest request, HttpServletResponse response, boolean[] isHandled) {
		...
		//add by dk 2017-06-15 begin
		if (target.endsWith(".do")) {
			target=target.replace(".do", "") ;
		}
		//add by dk end
		...
		try {
			Controller controller = SpringHolder.getBean(action.getControllerClass());
			
			//add by dk 2017-06-15 begin
			ThreadHolder.setHttpRequest(request);
			ThreadHolder.setHttpResponse(response);
