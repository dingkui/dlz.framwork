...
public class MyWsClient {
	...
	public MyWsClient(String url,IWsHandler handler){
		try {
			...
			throw new SystemException("WsClient 初始化有误 url:"+url, e);
		}
	...
	public void send(byte[] bytes) {
		if(status!=2){
			throw new SystemException("发送失败，status="+status);
		}
		client.send(bytes);
	}
	/**
	...
	public void connect(long retryTime,int connectTimeout) {
		if(status!=0 && status!=-1){
			throw new SystemException("连接失败，status="+status);
		}
	...
	public void reset() {
		if(status!=2){
			throw new SystemException("重置失败，status="+status);
		}
		client.close();
	}
	...
	public void close() {
		if(status!=1 && status!=2){
			throw new SystemException("关闭失败，status="+status);
		}
		status=-1;
		client.close();
	}
