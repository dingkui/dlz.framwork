		...
		for (int i = 0; i < propertyDescriptors.length; i++) {
			...
			if (map.containsKey(propertyName)) {
				...
				if(value.length==1){
					...
					logger.warn(obj.getClass().toString()+"."+descriptor.getWriteMethod().getName()+" invoke error:para:"+StringUtils.join(value, ","));
				}
			}
		}
		return obj;
	...
	private static void invoke(PropertyDescriptor descriptor,Object obj,String value){
		...
		if(vl!=null){
			try {
				...
				logger.warn(obj.getClass().toString()+"."+descriptor.getWriteMethod().getName()+" invoke error:para:"+vl);
			}
		}
	}
