...
> 内容以当前 v6.7.5 Java 源码和稳定 API 约定为准。
...
- `ICache` / `MemoryCache` / `CacheUtil` 提供**本地进程内内存缓存**，不跨 JVM 共享，不保证分布式一致性；
...
| 坐标     | `top.dlzio:dlz-kit:6.7.5`（GitHub: dingkui/dlz-kit）            |
...
6. 本地缓存按场景使用 `CacheUtil.get(...)` 或 `CacheMap.getAndSet(...)`；
