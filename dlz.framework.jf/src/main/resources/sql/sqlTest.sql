...
<sqlList>
	<!--
		测试
	-->
	<sql sqlId="key.sqlTest.getList"><![CDATA[
	    select AD_text,AD_ID,to_char(AD_ENDDATE,'yyyy') as AD_ENDDATE 
	    from JOB_AD t 
	...
	<sql sqlId="key.sqlTest.getStr"><![CDATA[
	   select to_char(AD_ENDDATE,'yyyy') as AD_ENDDATE from JOB_AD t where ad_id in (${ad_id})
 	]]></sql>
	<!--
		测试
	-->
